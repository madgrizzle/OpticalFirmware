...
void  setSpindlePower(bool powerState) {
    /*
     * Turn spindle on/off depending on powerState
     */
...
void laserOn(bool OnOff) {
  pinMode(LaserPowerPin, OUTPUT);
  if (OnOff) { // turn on Laser - EBS BETTER go LOW on system stop
            Serial.println(F("Turning Laser ON"));
            digitalWrite(LaserPowerPin, HIGH);
        }
        else {            // turn off Laser
           // Serial.println(F("Turning Laser OFF"));
